...
func _setup_players() -> void:
    ...
    for i in seat_count:
        ...
        if i == player_seat and player_seat >= 0:
            ...
            var bot_char: String = all_chars[_rng.randi_range(0, all_chars.size() - 1)]
...
func _pick_all_players_skill_indices() -> Dictionary:
    ...
    for p in _players:
        ...
        if not rev.is_empty() or not qs.is_empty():
...
func _skill_indices_for_seat(skill_by_seat: Dictionary, seat: int) -> Dictionary:
    ...
    if picked is Dictionary:
        return picked
    return {"revealed_indices": [], "quality_size_indices": []}
...
func _apply_single_effect_reveals(run_gen: int, effect: Dictionary) -> void:
    ...
    var can_reveal_visual: bool = icon_kind != "character" or str(effect.get("character_id", "")) == _human_character_id()
